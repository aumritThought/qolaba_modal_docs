...
def download_models():
    ...
    controlnet_tile = ControlNetModel.from_pretrained("ioclab/control_v1p_sd15_brightness", torch_dtype=torch.float16)

    pipe = StableDiffusionControlNetPipeline.from_pretrained(
        "Lykon/dreamshaper-8", controlnet=[controlnet, controlnet_tile], torch_dtype=torch.float16
    ).to('cuda')
...
class stableDiffusion:  
    def __enter__(self):
        ...
        self.pipe = StableDiffusionControlNetPipeline.from_pretrained(
            "Lykon/dreamshaper-8", controlnet=[controlnet, controlnet_tile], torch_dtype=torch.float16, safety_checker=None
        ).to('cuda')
    ...
    def run_inference(self, img, prompt,guidance_scale,negative_prompt,batch, strength, bg_img):
        ...
        image = self.pipe(prompt=prompt, image=[img,img],
            ...
            num_inference_steps=30, width=768, height=768, negative_prompt=negative_prompt, num_images_per_prompt=batch).images
