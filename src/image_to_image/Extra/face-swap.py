...
class stableDiffusion:
        ...
        with THREAD_LOCK:
            if self.FACE_ENHANCER is None:
                ...
                self.FACE_ENHANCER = GFPGANer(
                    model_path=model_path, upscale=1, device="cuda"
                )
    ...
    def run_inference(self, file_url, bg_img=None):
        ...
        if s_img == None:
            raise Exception(
                "Not able to fetch the second image to perform face swap",
                "Provide Proper Image URL",
            )
        else:
            try:
                ...
                if temp_frame.shape[1] >= temp_frame.shape[0]:
                    height = 1024
                    width = (
                        (int(temp_frame.shape[0] * 1024 / temp_frame.shape[1])) // 64
                    ) * 64
                else:
                    width = 1024
                    height = (
                        (int(temp_frame.shape[1] * 1024 / temp_frame.shape[0])) // 64
                    ) * 64
                temp_frame = cv2.resize(temp_frame, (height, width))
                reference_face = get_one_face(
                    temp_frame, roop.globals.reference_face_position
                )
                ...
                if roop.globals.many_faces:
                    ...
                    if roop.globals.many_faces:
                        for target_face in roop.globals.many_faces:
                            temp_frame = self.FACE_SWAPPER.get(
                ...
                raise Exception(
                    "Not able to detect face or Input image is invalid",
                    "Provide Proper Image",
                )
