...
class SafetyChecker:
    ...
    def check_nsfw_content(self, image: Imagetype) -> list[bool]:
        ...
        safety_checker_input = self.feature_extractor(
            np.array(image), return_tensors="pt"
        ).to("cuda")
...
def get_base_image() -> MIM:
    ...
    """
    return (
        MIM.debian_slim(python_version="3.11.8")
        .run_commands(BASE_IMAGE_COMMANDS)
        .pip_install_from_requirements(REQUIREMENT_FILE_PATH)
        .run_function(
            download_safety_checker, gpu="t4", secrets=[Secret.from_name(SECRET_NAME)]
        )
    )
...
def make_request(
    ...
    if method == "GET":
        ...
        response = requests.post(
            url, data=json_data, headers=headers, files=files, json=json
        )

    if response.status_code != 200:
        if (
            "content_moderation" in response.text
            or "content moderation" in response.text
        ):
...
# Cloudinary
def upload_data_gcp(
    data: Imagetype | str, extension: str, upscale: bool = False
) -> str:
    """
...
def upload_to_gcp(data: Imagetype | str, extension: str, upscale: bool = False) -> str:
    try:
        ...
        if upscale == True and extension == OUTPUT_IMAGE_EXTENSION:
            ...
            output = TaskResponse(
                **Model.run_inference.remote(
                    UpscaleParameters(
                        file_url=data, scale=4, check_nsfw=False
                    ).model_dump()
                )
            )
        ...
        storage_client = storage.Client(
            credentials=credentials, project=google_credentials_info["project_id"]
        )
