...
celery.conf.update(result_expires=CELERY_RESULT_EXPIRATION_TIME)
...
@celery.task(
    name="AI_task",
    time_limit=CELERY_RESULT_EXPIRATION_TIME,
...
def create_task(parameters: dict) -> dict:
    ...
    try:
        ...
        if parameters_model.app_id in service_registry.api_services:
            ...
            service_output = app_modal_instance.run_inference.remote(
            ...
            output_data = TaskResponse(**service_output)
            logger.info(
                f"Modal Service {parameters_model.app_id} executed successfully."
            )
            ...
            raise Exception(
                "Service Not Found",
                f"Given APP Id '{parameters_model.app_id}' is not available",
            )
        ...
        if output_data and output_data.extension is not None:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                full_res_futures = [
                    executor.submit(
                        ...
                        i,
                        img,
                        output_data.extension,
                        parameters_model.upscale,
                    )
                ...
                if output_data.extension == OUTPUT_IMAGE_EXTENSION:
                    if (
                        parameters_model.check_copyright_content
                        ...
                        copyrighted_content_futures = [
                            executor.submit(
                                identify_copy_righted_materials, urls[index], index
                            )
                    ...
                    low_res_futures = [
                        executor.submit(
                            upload_low_res_image, i, img, output_data.extension
                        )
                        for i, img in enumerate(output_data.result)
                    ]
            ...
            final_urls = [
                url
                for url, is_copyrighted in zip(urls, has_copyright_content)
                if not is_copyrighted
            ]
            final_low_res_urls = [
                url
                for url, is_copyrighted in zip(low_res_urls, has_copyright_content)
                if not is_copyrighted
            ]
        ...
        if output_data and output_data.time:  # Check if output_data and time exist
            if (
                output_data.time.runtime
                < time_required
                < (output_data.time.runtime + output_data.time.startup_time)
            ):
                output_data.time.startup_time = 0  # Adjust startup time if needed
            # Ensure runtime calculation is robust
            output_data.time.runtime = max(
                0, time_required - output_data.time.startup_time
            )
        else:
            # Handle case where output_data or output_data.time is None
            logger.warning(
                "Could not calculate detailed timing, output_data or output_data.time missing."
            )
        ...
        logger.exception(
            f"Task execution failed for app_id {parameters.get('app_id', 'N/A')}: {e}"
        )
        task_status = "FAILED"
