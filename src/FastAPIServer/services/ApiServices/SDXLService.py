...
class SDXL3Text2Image(IService):
    ...
    def generate_image(
        self, parameters: SDXL3APITextToImageParameters, model: str
    ) -> str:
        """
        ...
        if aspect_ratio == "3:7":
            aspect_ratio = "9:21"
        if aspect_ratio not in SDXL3_RATIO_LIST:
    ...
    def remote(self, parameters: dict) -> dict:
        ...
        """
        parameters: SDXL3APITextToImageParameters = SDXL3APITextToImageParameters(
            **parameters
        )
        ...
        for i in range(0, len(results)):
            ...
            if results[i][0] != None:
                image_urls.append(results[i][0])
        return prepare_response(
            image_urls, Has_NSFW_Content, 0, 0, OUTPUT_IMAGE_EXTENSION
        )
