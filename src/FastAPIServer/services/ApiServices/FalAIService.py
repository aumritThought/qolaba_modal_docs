from src.data_models.ModalAppSchemas import FluxText2ImageParameters, UpscaleParameters, IdeoGramText2ImageParameters, OmnigenParameters, Kling15Video, MinimaxVideo, HunyuanVideo, FluxImage2ImageParameters, RecraftV3Text2ImageParameters, SDXLText2ImageParameters, Kling2MasterParameters
...
from src.data_models.ModalAppSchemas import Veo2Parameters # Import the specific schema for Veo2
from src.utils.Globals import timing_decorator, prepare_response, make_request # Ensure these are imported
from src.FastAPIServer.services.IService import IService # Ensure IService is imported
from src.utils.Constants import OUTPUT_VIDEO_EXTENSION, IMAGE_GENERATION_ERROR, NSFW_CONTENT_DETECT_ERROR_MSG # Ensure constants are imported
import fal_client # Ensure fal_client is imported or available
import base64 # Ensure base64 is imported for potential use in other services
import io # Ensure io is imported for potential use in other services
import concurrent.futures # Ensure concurrent.futures is imported for potential use in other services
...
class Veo2(IService):
    ...
    def make_api_request(self, parameters : Veo2Parameters) -> bytes:
            ...
            try:
                ...
                logger.info(f"Fal Veo2 API request submitted. Handle type: {type(request_handle)}")

                # --- Fetch the final result from the handle ---
                final_response = request_handle.get()

                logger.info(f"Fal Veo2 API final result fetched. Type: {type(final_response)}")
                logger.debug(f"Fal Veo2 API final result content: {final_response}") # Log the actual result

                # --- Process the final_response dictionary ---
                if not isinstance(final_response, dict):
                    logger.error(f"Expected dict result from Fal Veo2 API handle, but got: {type(final_response)}")
                    raise Exception("Video generation failed", "Unexpected result type from API handle")

                video_info = final_response.get('video') # Access the key from the result dict
                if video_info and isinstance(video_info, dict) and 'url' in video_info:
                    ...
                    logger.info(f"Video content type: {type(video_content)}, length: {len(video_content)}")
                    return video_content
                else:
                    # Log the structure if it's unexpected
                    logger.error(f"Unexpected response structure or missing video URL from Fal Veo2 API: {final_response}")
                    raise Exception("Video generation failed", f"Unexpected API response structure or missing video URL")

            except Exception as e:
                 # Log the full exception trace for better debugging
                 logger.exception(f"Fal AI Veo2 API call, result fetching, or download failed: {e}")
    ...
    def remote(self, parameters: dict) -> dict:
            ...
            if isinstance(file_url_input, list) and len(file_url_input) > 0 and isinstance(file_url_input[0], dict):
                processed_params['file_url'] = file_url_input[0].get('uri')
                logger.debug(f"Extracted file_url from list: {processed_params['file_url']}")
            elif isinstance(file_url_input, dict): # Handle if just a dict is passed
                processed_params['file_url'] = file_url_input.get('uri')
                logger.debug(f"Extracted file_url from dict: {processed_params['file_url']}")

            # Ensure it's None if not a valid string URL after processing OR if it wasn't provided
            if not isinstance(processed_params.get('file_url'), str) or not processed_params.get('file_url'):
                processed_params['file_url'] = None # Set explicitly to None
                logger.debug("Processed file_url set to None as input was missing or not a valid string.")
            ...
            if isinstance(duration_input, (int, float)): # Handle numeric input (5, 5.0)
                processed_params['duration'] = f"{int(duration_input)}s" # Convert to int and format as string "Xs"
                logger.debug(f"Formatted numeric duration to string: {processed_params['duration']}")
            elif isinstance(duration_input, str) and duration_input.isdigit(): # Handle string digit input ("5")
                processed_params['duration'] = f"{duration_input}s" # Format as string "Xs"
                logger.debug(f"Formatted string-digit duration to string: {processed_params['duration']}")
            elif isinstance(duration_input, str) and duration_input.endswith('s'): # Already correct format ("5s")
                ...
                logger.warning(f"Unexpected duration type/format: {duration_input}. Passing to validation.")
            ...
            try:
                ...
                logger.error(f"Pydantic validation failed for Veo2 parameters after processing: {e}")
            ...
            # --- Prepare response ---
            logger.info(f"Type received from make_api_request in remote: {type(video_bytes)}")

            result_list = [video_bytes] if isinstance(video_bytes, bytes) else []
            if not result_list and isinstance(video_bytes, list): # Handle if make_api_request returned a list already
                result_list = video_bytes

            prepared_dict = prepare_response(
                result=result_list,
                Has_NSFW_content=[False] * len(result_list), # Assume false for now, Fal might not report this for video
...
class Kling2Master(IService):
    ...
    def make_api_request(self, parameters : Kling2MasterParameters) -> bytes:
        try:
            ...
            logger.error(f"Invalid duration format received: {parameters.duration}. Expected '5' or '10'.")
        ...
        try:
            ...
            final_response = request_handle.get()
            logger.debug(f"Kling2Master API final result fetched.")

            if not isinstance(final_response, dict):
                 logger.error(f"Expected dict result from Kling2Master API handle, but got: {type(final_response)}")
                 raise Exception("Video generation failed", "Unexpected result type from API handle")
            ...
            if video_info and isinstance(video_info, dict) and 'url' in video_info:
                ...
                for key, value in final_response.items():
                    if isinstance(value, dict) and 'url' in value and isinstance(value['url'], str):
                         ...
                         break
                if not video_url:
                    logger.error(f"Could not find any video URL in API response. Keys: {final_response.keys()}")
            ...
            if isinstance(e, fal_client.client.FalClientError):
                 try:
                     ...
                     if isinstance(details, list) and len(details) > 0 and isinstance(details[0], dict):
                         error_msg = f"Fal API Error: {details[0].get('msg', 'Unknown error')}"
                 except Exception:
                     pass
            raise Exception("Video generation failed", error_msg)
    ...
    @timing_decorator
    def remote(self, parameters: dict) -> dict:
        logger.info(f"Kling2Master Raw parameters received: {parameters.get('prompt', 'No prompt found')[:50]}...") # Log start with partial prompt
