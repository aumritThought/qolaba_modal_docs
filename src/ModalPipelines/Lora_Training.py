...
def download_models():
    ...
    os.system(
        f'python3 "finetune/make_captions.py" --batch_size="1" --num_beams="1" --top_p="0.9" --max_length="75" --min_length="5" --beam_search  --caption_extension=".txt" {train_data_path} --caption_weights="https://storage.googleapis.com/sfr-vision-language-research/BLIP/models/model_large_caption.pth"'
    )
    ...
    pipe = StableDiffusionXLPipeline.from_single_file(
        "RealismEngineSDXL.safetensors",
        torch_dtype=torch.float16,
        use_safetensors=True,
        variant="fp16",
    ).save_pretrained("Stable_diffusion")
...
@stub.cls(
    gpu="a100", memory=model_schema["memory"], container_idle_timeout=200, timeout=14400
)
class stableDiffusion:
    ...
    def download_zip_extract_and_delete(self, url, extract_folder):
        ...
        try:
            ...
            raise Exception(
                "Please provide accessible Zip file URL which only contains Images",
                "URL is not accessible",
            )
        if response.status_code == 200:
            random_folder_name = "".join(
            ...
            if non_image_found:
                ...
                raise Exception(
                    "Please provide zip file which only contains Images. There should not be inside another folder in zip."
                )
        else:
            raise Exception(
                f"Failed to download zip file. Status code: {response.status_code}",
                "Provide proper ZIP File which contains only Images",
            )
    ...
    def create_and_move_folder(self, source_path, repetition, token):
        ...
        for item in os.listdir(source_path):
            ...
            if (
                ...
                and os.path.splitext(item)[1].lower() in allowed_extensions
            ):
                shutil.move(item_path, os.path.join(new_folder_path, item))

    @method()
    def run_inference(
        self, file_url, max_steps, Image_repetition, token_string, category, epochs
    ):
        ...
        return {
            "result": [f"{train_file_name}_{random_string}.safetensors", token_string],
