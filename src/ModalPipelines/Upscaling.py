...
class stableDiffusion:
    ...
    def run_inference(self, parameters: dict) -> dict:
        ...
        upsampler_output, img_mode = self.upsampler.enhance(np_img[:, :, ::-1])

        if parameters.scale == 2:
            upsampler_output = cv2.resize(
                upsampler_output,
                (
                    int(upsampler_output.shape[1] / 2),
                    int(upsampler_output.shape[0] / 2),
                ),
            )
        ...
        images, has_nsfw_content = generate_image_urls(
            images, self.safety_checker, parameters.check_nsfw
        )
