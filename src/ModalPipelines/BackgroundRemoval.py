...
app.image = image.run_function(
    download_removal_model, secrets=[Secret.from_name(SECRET_NAME)]
)
...
class stableDiffusion:
    ...
    def run_inference(self, parameters: dict) -> dict:
        ...
        if parameters.blur == True:
            ...
            image = self.remover.process(
                parameters.file_url,
                type=str([parameters.r_color, parameters.g_color, parameters.b_color]),
            )
        else:
