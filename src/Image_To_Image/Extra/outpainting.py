...
def download_models():
    ...
    pipe = StableDiffusionControlNetInpaintPipeline.from_pretrained(
        "uf/epicrealism_pureEvolutionV3",
...
class stableDiffusion:
    ...
    @method()
    def run_inference(
        self, file_url, batch, right, left, top, bottom, prompt, negative_prompt
        ...
        def maskimage(image, right, left, top, bottom):
            ...
            extra_image = Image.new(
                image.mode, (image.size[0] - 64, image.size[1] - 64), (0, 0, 0)
            )
        ...
        init_image = init_image.resize(
            (64 * round(init_image.size[0] / 64), 64 * round(init_image.size[1] / 64))
        )
        if (
            init_image.size[0] > 1024
            or init_image.size[0] < 256
            or init_image.size[1] > 1024
            or init_image.size[1] < 256
        ):
            if init_image.size[1] >= init_image.size[0]:
                height = 1024
                width = (
                    (int(init_image.size[0] * 1024 / init_image.size[1])) // 64
                ) * 64
            else:
                width = 1024
                height = (
                    (int(init_image.size[1] * 1024 / init_image.size[0])) // 64
                ) * 64
