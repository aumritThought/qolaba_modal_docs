...
def test_prompt_parrot_remote(mocker):
    ...
    mocker.patch('src.utils.Globals.prepare_response', return_value={"result": ["Enhanced prompt"]})
    ...
    mocker.patch('src.FastAPIServer.services.ApiServices.ClaudeAIService.BASE_PROMPT_FOR_GENERATION', "Base prompt template: ")
    
    # Act
    result = service.remote(parameters)
    
    # Assert
    from src.utils.Globals import prepare_response
...
def test_video_prompt_parrot_remote(mocker):
    ...
    service = VideoPromptParrot()
    mocker.patch.object(service, 'generate_prompt', return_value="Enhanced video prompt")
    mocker.patch('src.utils.Globals.prepare_response', return_value={"result": ["Enhanced video prompt"]})
    mock_executor = MagicMock()
    mock_executor.__enter__.return_value.submit = MagicMock()
    mock_future = MagicMock()
    ...
    # Mock the imported constants
    mocker.patch('src.FastAPIServer.services.ApiServices.ClaudeAIService.BASE_PROMPT_FOR_VIDEO_GENERATION', "Base video prompt template: ")
