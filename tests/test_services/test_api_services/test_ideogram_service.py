...
def test_ideogram_text2image_remote(mocker, mock_response):
    ...
    mocker.patch(
        "src.FastAPIServer.services.ApiServices.IdeogramService.IdeoGramText2Image.make_api_request",
    ...
    mocker.patch(
        ...
        return_value={"images": ["test_image_data"]},
    )
...
def test_ideogram_remix_remote(mocker, mock_response):
    ...
    mocker.patch(
        "src.FastAPIServer.services.ApiServices.IdeogramService.IdeogramRemix.make_api_request",
        return_value=mock_response.content,
    )
    mocker.patch("src.utils.Globals.convert_to_aspect_ratio", return_value="1:1")
    mocker.patch(
