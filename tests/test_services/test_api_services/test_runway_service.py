...
def test_download_and_convert_to_base64(mocker):
    ...
    mocker.patch(
        "src.FastAPIServer.services.ApiServices.RunWayService.get_image_from_url",
...
def test_generate_image_success(mocker):
    ...
    mocker.patch(
        "src.FastAPIServer.services.ApiServices.RunWayService.make_request",
...
def test_generate_image_no_images_error(mocker):
    ...
    params = RunwayImage2VideoParameters(
        prompt="test prompt", aspect_ratio="1280:768", duration=5, batch=1, file_url=[]
    )
...
def test_generate_image_processing_failure(mocker):
    ...
    mock_task_failed.model_dump.return_value = {
        "status": "FAILED",
...
def test_remote_with_concurrent_execution(mocker):
    ...
    mocker.patch.object(
        RunwayImage2Video, "generate_image", return_value=b"video_content"
    )
