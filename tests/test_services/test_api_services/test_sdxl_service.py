...
def test_sdxl3_generate_image_success(mocker):
    ...
    mocker.patch(
        "src.FastAPIServer.services.ApiServices.SDXLService.make_request",
    ...
    parameters = SDXL3APITextToImageParameters(
        prompt="test prompt", width=1024, height=1024, batch=1, negative_prompt=""
    )
...
def test_sdxl3_generate_image_invalid_aspect_ratio(mocker):
    ...
    mocker.patch.dict("os.environ", {"SDXL_API_KEY": "test_key"})
    mocker.patch(
        "src.utils.Globals.convert_to_aspect_ratio", return_value="5:7"
    )  # Invalid ratio
    ...
    # Create parameters
    parameters = SDXL3APITextToImageParameters(
        prompt="test prompt", width=500, height=700, batch=1, negative_prompt=""
    )
...
def test_sdxl3_remote_success(mocker):
    ...
    mock_generate_image = mocker.patch.object(
        SDXL3Text2Image, "generate_image", return_value=[b"test_image_data", False]
    )

    # Mock the prepare_response function
    expected_response = {
        "result": [b"test_image_data", b"test_image_data"],
        "Has_NSFW_Content": [False, False],
        "extension": "webp",
        "time": {"startup_time": 0, "runtime": 1},
    }
...
def test_sdxl3_remote_with_nsfw_content(mocker):
    ...
    mock_generate_image = mocker.patch.object(
        SDXL3Text2Image, "generate_image", return_value=[None, True]
    )
