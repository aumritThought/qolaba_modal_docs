...
from src.utils.Constants import DALLE_SUPPORTED_HW, NSFW_CONTENT_DETECT_ERROR_MSG, IMAGE_GENERATION_ERROR
...
def test_make_dalle_api_request_success(mocker):
    ...
    mocker.patch('src.FastAPIServer.services.ApiServices.OpenAIService.make_request', 
    ...
    mock_client.images.generate.assert_called_once_with(
        model="dall-e-3", prompt="test prompt", size="1024x1024", quality="standard", n=1
    )
...
def test_remote_success(mocker):
    ...
    mocker.patch.object(
        DalleText2Image, 
        'make_dalle_api_request', 
        return_value=b"fake-image-content"
    )
...
def test_gemini_init(mocker):
    ...
    mocker.patch('google.auth.load_credentials_from_dict', return_value=(mock_credentials, mock_project_id))
    ...
    # Verify
    from src.FastAPIServer.services.ApiServices.OpenAIService import COPYRIGHT_DETECTION_FUNCTION_CALLING_SCHEMA
    assert service.function_calling_schema == COPYRIGHT_DETECTION_FUNCTION_CALLING_SCHEMA
