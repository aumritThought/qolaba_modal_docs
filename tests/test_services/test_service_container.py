...
async def test_list_apps(mocker):
    ...
    mocker.patch("src.FastAPIServer.services.ServiceContainer._Client.from_env", 
                 return_value=mock_client)
    
    # Mock the environment
    mock_env = "test_env"
    mocker.patch("src.FastAPIServer.services.ServiceContainer.ensure_env", 
...
def test_register_internal_services(mocker):
    ...
    class MockService1(IService):
        def remote(self, data): pass
        
    class MockService2(IService):
        def remote(self, data): pass
    
    # Mock IService.__subclasses__ to return our mock classes
    mocker.patch("src.FastAPIServer.services.ServiceContainer.IService.__subclasses__", 
                 return_value=[MockService1, MockService2])
    
    # Mock list_apps function
    mock_apps = ["app1", "app2"]
    mocker.patch("src.FastAPIServer.services.ServiceContainer.list_apps", 
                 return_value=mock_apps)
    
    # Mock Cls.lookup to return a mock model
    mock_model = MagicMock()
    mocker.patch("src.FastAPIServer.services.ServiceContainer.Cls.lookup", 
