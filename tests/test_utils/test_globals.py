...
from src.utils.Globals import (
    convert_image_to_bytes, prepare_response, make_request, upload_data_gcp, 
    upload_to_gcp, resize_image, get_image_from_url, invert_bw_image_color,
    simple_boundary_blur, get_seed_generator, compress_image, timing_decorator,
    check_token, get_clean_name, convert_to_aspect_ratio
)
...
def test_make_request(mocker):
    ...
    response = make_request("https://example.com", "GET", headers={"Accept": "application/json"})
    mock_get.assert_called_once_with("https://example.com", headers={"Accept": "application/json"})
    ...
    mock_post.assert_called_once_with("https://example.com", data=json_data, headers=None, files=None, json=None)
...
def test_get_image_from_url(mocker):
    ...
    # Test error handling
    mocker.patch('src.utils.Globals.make_request', side_effect=Exception("Network error"))
...
def test_invert_bw_image_color(mocker):
    ...
    mock_from_array = mocker.patch('PIL.Image.fromarray', return_value=Image.new('L', (10, 10), color=255))
