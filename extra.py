...
from diffusers import AutoencoderKL, EulerDiscreteScheduler, UNet2DConditionModel, DiffusionPipeline
...
class StableDiffusionAITPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin):
    r"""
...
#         )
        ...
        self.vae_scale_factor = 2 ** (len(self.vae_ait_exe.config.block_out_channels) - 1)
    ...
    def __call__(
        ...
        if not output_type == "latent":
            image = self.vae_inference.decode(latents / self.vae.config.scaling_factor,height, width)[0]
        else:
            image = latents
