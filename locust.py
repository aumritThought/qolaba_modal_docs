...
#         if os.path.exists('4xESRGAN.pth'):
#             shutil.move('4xESRGAN.pth', self._get_file_path('4xESRGAN.pth'))
#         sd = torch.load(self._get_file_path('4xESRGAN.pth'))
...
image_path = '/home/prakhar-pc/qolaba/Modal-Deployments/file_ylasmb.jpg'
...
# Display the shape of the tensor (channels, height, width)
print("Image shape:", torch_image.shape)


def test() :
    sd = torch.load("/home/prakhar-pc/qolaba/Modal-Deployments/4x-UltraSharp.pth")
    in_nc, out_nc, nf, nb, plus, mscale = infer_params(sd)
    model = RRDBNet(in_nc=in_nc, out_nc=out_nc, nf=nf, nb=nb, upscale=mscale, plus=plus)
    model.load_state_dict(sd)
    model.eval()
    img = torch.zeros(1, 3, 128, 128, dtype = torch.float32)
    ret = model(torch_image)
    print(ret.shape)
    print(ret.dtype)
    print('done')
