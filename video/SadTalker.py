...
class stableDiffusion:    
    ...
    def run_inference(self,file_url, audio_url):
        ...
        try:
            if(file_url.size[1]>=file_url.size[0]):
                height=512
                width=((int(file_url.size[0]*512/file_url.size[1]))//64)*64
            else:
                width=512
                height=((int(file_url.size[1]*512/file_url.size[0]))//64)*64
                    
            file_url=file_url.resize((height, width))
